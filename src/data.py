...
class DETRData(Dataset): 
    def __init__(self, path, train=True):
        ...
        # Initialize base transform for fallback
        ...
        # Initialize augmented transform
    ...
    def safe_transform(self, image, bboxes, labels, max_attempts=50):
        for attempt in range(max_attempts):
            try:
                transformed = self.augment_transform(image=image, bboxes=bboxes, class_labels=labels)
        ...
        # Fallback to base transform if augmentation fails or loses all boxes
        return self.base_transform(image=image, bboxes=bboxes, class_labels=labels)
